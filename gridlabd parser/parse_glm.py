...
class GlmParser:
    ...
    def extract_attr(self, attr_str, src_str):
        ...
        attr_list = re.findall(self.re_glm_attr_tpl_str.format(
            attr_str), src_str, flags=re.DOTALL)
        return attr_list
    ...
    def parse_node(self, lines_str):
        ...
        """
        self.all_nodes_list = self.extract_obj(
            self.re_glm_syn_obj_node, lines_str)
        ...
        for cur_obj_str in self.all_nodes_list:
            ...
            assert len(
                cur_nd_obj_name_list) == 1, 'Redundancy or missing on the name attribute!'
            ...
            assert len(
                cur_nd_obj_phases_list) == 1, 'Redundancy or missing on the phase attribute!'
            self.all_nodes_phases_dict[cur_nd_obj_name_list[0]
    ...
    def parse_load(self, lines_str):
        ...
        for cur_obj_str in self.all_loads_list:
            ...
            assert len(
                cur_ld_obj_name_list) == 1, 'Redundancy or missing on the name attribute!'
            self.all_loads_names_list.append(cur_ld_obj_name_list[0])

            # ==Phases
            cur_ld_obj_phases_list = self.extract_attr('phases', cur_obj_str)
            assert len(
                cur_ld_obj_phases_list) == 1, 'Redundancy or missing on the phase attribute!'
            self.all_loads_phases_dict[cur_ld_obj_name_list[0]
                                       ] = cur_ld_obj_phases_list[0]

            # ==P & Q
            cur_ld_obj_sabc = ['']*3
            #cur_ld_obj_sabc[0] = re.findall(r'.*constant_power_A\s*(.*?);',cur_obj_str,flags=re.DOTALL)
            cur_ld_obj_sabc[0] = self.extract_attr(
                'constant_power_A', cur_obj_str)
            ...
            for cur_ite in range(len(cur_ld_obj_sabc)):
                if cur_ld_obj_sabc[cur_ite]:
                    cur_ld_obj_pabc[cur_ite] = complex(
                        cur_ld_obj_sabc[cur_ite][0]).real
                    cur_ld_obj_qabc[cur_ite] = complex(
                        cur_ld_obj_sabc[cur_ite][0]).imag
    ...
    def export_glm_with_gfas(self, output_glm_path_fn,
        ...
        for ld_str, gp in zip(self.all_loads_list, all_loads_ufls_tag):
            ...
            if flag_triload:
                hf_output.write(
                    self.obj_triload_tpl_str.format(ld_str, cur_gfa_str))
            else:
                hf_output.write(
                    self.obj_load_tpl_str.format(ld_str, cur_gfa_str))
        hf_output.close()
    ...
    def adjust_load_amount(self, load_glm_path_fn, adj_load_glm_path_fn, tgt_p, tgt_pf):
        ...
        print(
            f'Total Load after Adjustment: {p_ratio*self.all_loads_p_sum/1e3} (kW)')
        ...
        for cur_obj_str in self.all_loads_list:
            ...
            out_adj_load_str += self.obj_load_tpl_str.format(
                cur_new_obj_str, '')
    ...
    def update_pq(self, cur_obj_str, p_ratio, tgt_pf):
        # print(cur_obj_str)
        power_atts_list = ['constant_power_A',
                           'constant_power_B', 'constant_power_C']
        cur_new_obj_str = cur_obj_str
        for cur_att_str in power_atts_list:
            cur_att_re_str = fr".*{cur_att_str}\s*(.*?);"
            cur_ph_s_lt = re.findall(
                cur_att_re_str, cur_new_obj_str, flags=re.DOTALL)
            if cur_ph_s_lt:
        ...
        with open(obj_zone_csv_path_fn) as csvfile:
            ...
            for cur_row in obj_zone_csv_reader:
                ...
                if not cur_obj_zone_int:
                    ...
                    assert len(
                        cur_obj_zone_int) == 1, 'Redundancy of Segment Info'
...
def test_add_ufls_gfas():
    ...
    p.add_ufls_gfas(output_glm_path_fn, ufls_pct, ufls_th,
                    ufls_dly, gfa_rc_time, gfa_extra_str)
...
def test_mapping_zone_info():
    ...
    for cur_node_name_str in p.all_nodes_names_list:
        ...
        if cur_node_name_key in node_zone_dict.keys():
            ...
            new_node_zone_phase_dict[cur_node_name_str] = [
                cur_zone_info, cur_phase_str]
    ...
    for cur_load_name_str, cur_load_p, cur_load_q in zip(p.all_loads_names_list, p.all_loads_p_list, p.all_loads_q_list):
        ...
        if cur_load_name_key in load_zone_dict.keys():
            ...
            new_load_zone_phase_dict[cur_load_name_str] = [
                cur_zone_info, cur_phase_str]
